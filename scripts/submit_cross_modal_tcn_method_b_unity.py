#!/usr/bin/env python3
"""Submit a short-QOS Method B TCN/InfoNCE InFOM diagnostic on Unity."""
...
def jax_preflight(run_root: Path) -> str:
  ...
  --output="{run_root}/probe_results/method_b_${{SLURM_JOB_ID}}.json"
code=$?
if [ "$code" != "0" ]; then
...
{resume_checkpoint_path_arg}{checkpoint_dir_arg}  --checkpoint_at_end={args.checkpoint_at_end} \\
  ...
  --agent.shared_latent_dim={args.shared_latent_dim} \\
...
"""
    return f"""#!/bin/bash
#SBATCH --job-name=tcn-infom-t{args.task}-s{args.seed}
#SBATCH --partition={args.partition}
{optional_sbatch("qos", args.qos)}{optional_sbatch("dependency", args.dependency)}{optional_sbatch("constraint", args.constraint)}{optional_sbatch("gres", args.gres)}#SBATCH --cpus-per-task={args.cpus}
#SBATCH --mem={args.mem}
...
def main() -> int:
    ...
    parser.add_argument("--constraint", default="h100|a100-80g")
    ...
    parser.add_argument("--wandb-run-group", default="cross_modal_tcn_method_b_diagnostic")
    parser.add_argument("--resume-checkpoint-path", type=Path, default=None)
    parser.add_argument("--checkpoint-dir", type=Path, default=None)
    parser.add_argument("--pretraining-steps", type=int, default=25_000)
    parser.add_argument("--pretraining-size", type=int, default=1_000_000)
    parser.add_argument("--finetuning-steps", type=int, default=0)
    parser.add_argument("--finetuning-size", type=int, default=500_000)
    parser.add_argument("--batch-size", type=int, default=256)
    parser.add_argument("--shared-latent-dim", type=int, default=512)
    ...
    parser.add_argument("--device-bridge-cache", dest="device_bridge_cache", action="store_true", default=True)
    parser.add_argument("--no-device-bridge-cache", dest="device_bridge_cache", action="store_false")
    parser.add_argument("--dry-run", action="store_true")
    args = parser.parse_args()

    timestamp = dt.datetime.now().strftime("%Y%m%d_%H%M%S")
    run_root = args.remote_run_root or (
        DEFAULT_BASE / f"infom-learning-from-obs/runs/cross_modal_tcn_method_b_diag_{timestamp}"
    )
    local_root = args.local_run_root / f"cross_modal_tcn_method_b_diag_{timestamp}"
